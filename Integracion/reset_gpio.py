...
def turn_90_degrees(pwm_left_speed, pwm_right_speed):
    ...
    rospy.loginfo(f"Inicio de la curva: Pulsos deseados izq: {pulsos_deseados_izq}, der: {pulsos_deseados_der}")
    
    while encoder_left_count < pulsos_deseados_izq or encoder_right_count < pulsos_deseados_der:
        rospy.loginfo(f"Pulsos actuales: izq: {encoder_left_count}, der: {encoder_right_count}")
        time.sleep(0.001)  # Ajusta este valor según sea necesario
...
def main():
    ...
    while not rospy.is_shutdown():
        rospy.loginfo("Waiting for message on /robot_status/started...")
        data = rospy.wait_for_message('/robot_status/started', String)
        if data.data == "STARTED":
            rospy.loginfo("Received STARTED message, starting reset_gpio.py.")
            start_l_turn_movement()
        else:
             rospy.logerr("Failed to execute reset_gpio.py")

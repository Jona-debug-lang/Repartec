...
def obstacles_callback(data):
    ...
    for circle in data.circles:
        if circle.velocity.x ** 2 + circle.velocity.y ** 2 > 0.01  ** 2:
            # Verificar si el objeto en movimiento está dentro del radio de 100 cm
            if (circle.center.x ** 2 + circle.center.y ** 2) <= 1.0 ** 2:
                should_stop = True
...
def move_straight(distancia_cm, pwm_left_speed, pwm_right_speed, eje):
    global start_time, end_time, encoder_left_count, encoder_right_count, deviation_y, deviation_x, should_stop

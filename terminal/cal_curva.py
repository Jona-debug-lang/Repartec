...
def spin_in_place(duration):
    ...
    pwm_izq.ChangeDutyCycle(5)  # PWM constante para el motor izquierdo
    pwm_der.ChangeDutyCycle(100)  # PWM constante para el motor derecho
    
    count_izq = 0
    count_der = 0
    start_time = time.time()
    
    print("Inicio del giro")
    ...
    while True:
        ...
        if elapsed_time >= duration:
            break
        print(f"Tiempo transcurrido: {elapsed_time:.2f} segundos, Pulsos actuales: izq: {count_izq}, der: {count_der}")
...
def main():
    try:
        # Definir la duración inicial del giro (en segundos)
        duration = 5  # Ajustar este valor según sea necesario

        # Realizar el giro en su propio eje
        spin_in_place(duration)
